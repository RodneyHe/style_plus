...
class AttrEncoder(Model):
    ...
    def my_save(self, reason=""):
        self.model.save_weights(str(self.args.weights_dir.joinpath(self.__class__.__name__ + f"_{reason}" + '.h5')))
    
    def my_load(self, reason=""):
        self.model.built = True
        self.model.load_weights(str(self.args.weights_dir.joinpath(self.__class__.__name__ + f"_{reason}" + '.h5')))
