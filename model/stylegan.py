...
class Truncation(Layer):
    ...
    def build(self, input_shape):
        self.dlatent_avg = self.add_weight('dlatent_avg', shape=[int(input_shape[-1])])
...
def StyleGAN_G_synthesis(dlatent_size=512, resolution=1024, is_const_noise=True):
    ...
    clatents_in = tf.keras.layers.Input(shape=[9984], name='G_synthesis/clatents_in') # controling latents input

    res_list = [1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 512, 512, 256, 256, 128, 128]
    ...
    def layer_epilogue(x, layer_idx, name):
        ...
        style_ = style + clatents_in[:, sum(res_list[:layer_idx]):sum(res_list[:layer_idx+1])]
        style_list.append(style_)

        x = StyleModApply(name=name+'StyleModApply')([x, style_])
    ...
    return Model(inputs=[dlatents_in, clatents_in], outputs=[x, style_list, noise_inputs], name='G_synthesis')
...
class StyleGAN_G(Model):
    ...
    def call(self, inputs, clatents):
        x = self.model_mapping(inputs)
        x = self.model_synthesis([x, clatents])
        return x
